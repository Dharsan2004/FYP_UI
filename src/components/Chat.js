    ...
    <div className="w-full h-[85%] flex items-center justify-center overflow-hidden overflow-y-auto px-2 py-1 scroll">
      <div className="w-full lg:w-4/5 flex flex-col h-full items-start justify-start">
        {message?.map((msg, i) => (
          <span
            key={i}
            className={`flex items-start justify-center gap-2 lg:gap-5 my-2 p-3 rounded-md 
              ${msg.isBot ? "lg:bg-gray-800/80 bg-gray-200" : "bg-white"} 
              ${msg.isBot ? "lg:text-white text-black" : "text-black"}`}
          >
            <img
